...
public final class HttpUtils {
	...
	public static String httpPostByNullXml(String url) {
		...
		try {
			...
			logger.error(String.format("[httpPostByJson has exception] error url is %s, the params is %s", url, null));
			return null;
		} catch (SocketTimeoutException e) {
			e.printStackTrace();
			logger.error(String.format("[httpPostByJson has exception] error url is %s, the params is %s", url, null));
			...
			e.printStackTrace();
			logger.info(String.format("[httpPostByJson has exception] error url is %s, the params is %s", url, null));
			return null;
		} finally {
			if (httpPost != null) {
				httpPost.releaseConnection();
