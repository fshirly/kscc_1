	...
	function setLiveChoose(){
	 ...
	 function setUploadDiv(uploadBtn){
		 $("#"+uploadBtn).off().on("click",function(){
			 ...
			 if($(this).html()=="提交"){
				 if($(parents).attr("editStatus")!=1&&(!$(parents).find("input[type='file']").val()||!$(parents).find("select").val())){
					 var tips = "";
					 if(!$(parents).find("input[type='file']").val()){
						 //如果没有图片
						 tips = "请先上传图片！";
					 ...
					 buttons: [
						 {
							 ...
							 clickEvent: function () {
								 ...
								 //清空图片
								 $(_this).parents("form").find(".imgPreview img").attr({"src":$.base+"/images/no-image.png"});
								 //清空url链接
        ...
        $("#"+id).unbind().on("change",function() {
			...
			reader.onload = function(theFile) {
				...
				image.onload = function() {
					if(this.width!=1140||this.height!=380){
						var modal = base.modal({
							label: "提示",
							context: "<div style='text-align:center;font-size:13px;'>建议上传宽度为1140px且高度为380px的图片，确定继续上传吗？</div>",
							width: 250,
							height: 80,
							buttons: [
								{
